#!/usr/bin/env python3
"""Arm C tool for EXP-002: the no-iterative-feedback designer.
...
nothing this arm's session can read is influenced by that evaluation.
    ...
    brief     print requirements, materials, prior constellation (Core-scored,
              the same fixed prior every arm receives), and the one-shot rule
...
def cmd_finish(args):
    ...
    for row in notes:
        lines.append(f"| {row['candidate_id']} | {common.layers_text(row['layers'])} | {row['rationale']} |")
