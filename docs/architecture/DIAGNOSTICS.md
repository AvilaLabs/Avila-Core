...
| `CORE-E7401` | Review decision not admissible | SC-9 and SC-11 A9 | A decision names a step with no compiled review obligation, carries a disposition the review type does not allow, or repeats a step's decision. Decisions are unverified assertions in this slice; they withhold `PASS` until present and never become a technical verdict. | Record one allowed disposition per review step. Signature and eligibility verification are later admission checks. |
...
| `CORE-T2102` | Kind mismatch | SC-1 and SC-6 R6 | A requirement's `limit` or `tolerance` names a quantity kind different from the metric role's kind, or the metric role is not a quantity role at all. | Set the kind to the metric role's kind, or choose a quantitative metric. |
