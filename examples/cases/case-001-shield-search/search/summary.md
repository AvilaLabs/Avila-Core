...
This frozen search predates the optional practical-review slice, so its historical log contains no review records. Under the now-bound instruction to request changes unless every Core requirement is `PASS`, all three finalists would be returned (two are `INCONCLUSIVE`, one is `FAIL`) and none would enter the user-presentation queue. Current runs of `shield_search.py` record that routing step explicitly. The stage controls presentation only and never alters Core's verdicts.
