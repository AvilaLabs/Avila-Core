...
| --- | --- | --- | --- |
| Canonical semantic kernel | Partial | 103 pure vectors: canonical values, exact unit scaling, three-valued predicates, and four-state verdict calculus including every categorical `equals`/`in_set` edge and the mixed admitted+non-admitted `not_evaluated` ordering | Total archive/package canonicalization, remaining predicate and aggregation boundaries, and independent implementation |
...
| Diagnostic contract | Implemented for current compiler slice | 34 catalogued finding codes, anchored JSON Pointers, typed RFC 6902 repairs, mutation tests, and a 23-case real-contract defect corpus pinning status, findings, and snapshot identity for seeded defects in the committed CASE-001 pair | Fixture coverage checker and measured false-block/clarification outcomes |
...
| Planning and selection | Planned | Static type satisfiability plus bound plans — `run --plan` emits a content-identified `bound-plan/v0.1-draft` with a closed per-step decision vocabulary (`reuse_committed`/`execute`/`blocked`/`refused`/`not_executed`) that verifies declared capability bytes and names unmet roots, capabilities, and environment keys — and deterministic capability discovery: `--capability-dir` scans a directory and binds the file whose digest equals the manifest's `executable_sha256` pin, with the resolution source recorded on the bound plan | Multi-implementation ranking policy, package discovery across registries, and estimates |
...
| Question-first workbench | Partial | The case workbench renders every stage of a run and the specimen compiler now does JSON-level preflight editing — open a contract/registry pair, recompile on demand, apply exact repair candidates, save explicitly; form-based authoring and testing with pilot participants remain |
