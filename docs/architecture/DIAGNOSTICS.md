...
| `CORE-E8010` | Type mismatch | ENGINEERING_LANGUAGE §5 and §7.B | An argument's quantity kind does not match the declared slot, an argument's claim does not satisfy the slot's claim model, or two arguments bind the same signature variable to different entities — scenario identity is unified by entity identity, not scope equality. | Supply an argument of the declared kind, claim, and relation binding. Different scenarios under the same scope are still different scenarios. |
