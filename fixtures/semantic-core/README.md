...
`derivations/` pins verdict-derivation records (ADR-0026) generated by
`avila-core evaluate --derivation` over `examples/cases/case-000-actinv-
aftermatter`'s committed documents, for the Python verifier's replay tests
(`verifier/test_verifier.py::TestDerivationReplay`):

| File | What it pins |
| --- | --- |
| `case-000.derivation.json` / `.campaign-report.json` | the honest digest-only replay — every recorded application verifies |
| `case-000-observed.derivation.json` / `.campaign-report.json` / `.artifact.bin` | an evaluation with `--artifact` bytes — the bound `artifact_observations` replay to `verified` |
| `case-000-stale-qualification.*` | an expired envelope on the claim CASE-000-R1 consumes — replays to `not_evaluated.qualification_expired` |
| `case-000-mixed-qualification.*` | a qualification both `expired` and `revoked_by`-refused — replays to `not_evaluated.qualification_revoked` carrying both `CORE-A4603` and `CORE-A4602` |
