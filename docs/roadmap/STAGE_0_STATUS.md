...
2. Strict qualification, landed in mechanism (S-035) with candidate validation
   at the design boundary (S-036). Remaining: an honest, limited qualification
   record for the screen capabilities' geometry arithmetic (mass, thickness,
   areal mass) so `require_qualification` can be set on CASE-001, CASE-002 and
   CASE-003 without turning those bounded requirements `NOT_EVALUATED`.
3. Runner integrity fixes: process-group kill on timeout and one locked write
   per campaign-log line (S-037) and the opt-in verified-hash cache (S-038) are
   landed. Remaining: extraction of rendering and logging from the case runner
   without behaviour change, and adversarial tests pinning that registry and
   policy edits re-evaluate over reused evidence.
