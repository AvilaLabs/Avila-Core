...
pub const DIAGNOSTIC_CATALOG: &[DiagnosticExplanation] = &[
    ...
    DiagnosticExplanation {
        code: "CORE-E8001",
        title: "Malformed language document",
        rule: "ENGINEERING_LANGUAGE §4 and §10.1",
        meaning: "A program or library document failed admission: the bytes are not authoritative canonical JSON, a field appears at a position the schema does not declare — including an annotation-named field at an undeclared position — a field's value has the wrong shape, or a schema-directed projection could not be formed.",
        next_action: "Correct the document to the declared schema. Admission precedes identity; nothing downstream analyzes an inadmissible document.",
    },
    DiagnosticExplanation {
        code: "CORE-E8002",
        title: "Library pin mismatch",
        rule: "ENGINEERING_LANGUAGE §10.1",
        meaning: "The program pins a library by semantic digest and the supplied library projects to a different identity, or names a different name or revision. A semantic edit mints a new identity; the old context refuses it rather than analyzing under rules that changed silently.",
        next_action: "Supply the library the pin names, or re-pin the program against the intended library revision.",
    },
    DiagnosticExplanation {
        code: "CORE-E8003",
        title: "Undeclared proposition",
        rule: "ENGINEERING_LANGUAGE §4",
        meaning: "An assumption, premise, or requirement names a proposition the library does not declare. Propositions are a closed set; an unresolvable identifier is a finding, not a fresh symbol.",
        next_action: "Declare the proposition in the library's `propositions` map with its parameter list, or correct the reference.",
    },
    DiagnosticExplanation {
        code: "CORE-E8004",
        title: "Undeclared method",
        rule: "ENGINEERING_LANGUAGE §4",
        meaning: "An `apply` names a method identifier the bound library does not declare.",
        next_action: "Declare the method in the library, or correct the `apply` to name a declared method.",
    },
    DiagnosticExplanation {
        code: "CORE-E8005",
        title: "Undeclared entity",
        rule: "ENGINEERING_LANGUAGE §5",
        meaning: "A relation binds an entity identifier the program does not declare under that relation kind.",
        next_action: "Declare the entity in `entities` under the right relation, or correct the reference.",
    },
    DiagnosticExplanation {
        code: "CORE-E8010",
        title: "Type mismatch",
        rule: "ENGINEERING_LANGUAGE §5 and §7.B",
        meaning: "An argument's quantity kind does not match the declared slot, an argument's claim does not satisfy the slot's claim model, or two arguments bind the same signature variable to different entities — scenario identity is unified by entity identity, not scope equality.",
        next_action: "Supply an argument of the declared kind, claim, and relation binding. Different scenarios under the same scope are still different scenarios.",
    },
    DiagnosticExplanation {
        code: "CORE-E8011",
        title: "Required relation absent",
        rule: "ENGINEERING_LANGUAGE §2.2 and §5",
        meaning: "A slot requires a relation key the argument does not carry. An absent relation key means `not carried`, never a wildcard; the slot cannot unify over a dimension the value lacks.",
        next_action: "Supply an argument carrying the declared relation, or redeclare the slot's relation set if the dimension is genuinely absent.",
    },
    DiagnosticExplanation {
        code: "CORE-E8012",
        title: "Coverage obligation unmet",
        rule: "ENGINEERING_LANGUAGE §7.B and §8.1",
        meaning: "The requirement declares a `scope` or `scenario` the subject's scenario entity does not satisfy — a steady-state result asked to serve a transient requirement fails the scope check, and a subject with no `scenario` relation cannot establish scope at all.",
        next_action: "Derive the subject under a scenario whose scope satisfies the requirement, or relax the requirement's declared scope.",
    },
    DiagnosticExplanation {
        code: "CORE-E8013",
        title: "Contradiction",
        rule: "ENGINEERING_LANGUAGE §8",
        meaning: "Two program assumptions assert and deny the same proposition at the same scope. Dependent uses are blocked; a contradiction never discharges vacuously.",
        next_action: "Resolve the contradiction in `assumptions` — assert or deny, not both.",
    },
    DiagnosticExplanation {
        code: "CORE-E8014",
        title: "Premise conflict",
        rule: "ENGINEERING_LANGUAGE §8.2 and §7.E4",
        meaning: "A premise asserts a proposition the recorded context itself refutes — `provenance_disjoint` over inputs whose recorded source edges intersect, or a proposition the program denies. The premise is inadmissible and discharges nothing.",
        next_action: "Remove the asserted premise, or supply the inputs under genuinely disjoint recorded sources.",
    },
    DiagnosticExplanation {
        code: "CORE-E8015",
        title: "Cyclic witness",
        rule: "ENGINEERING_LANGUAGE §8 A3",
        meaning: "A premise's support transitively requires the proposition it asserts — a self-dependent witness or a mutual cycle. Discharge is refused; the assumption stays residual and the conditional verdict stands.",
        next_action: "Break the cycle by grounding one premise in evidence that does not return to the asserted proposition.",
    },
    DiagnosticExplanation {
        code: "CORE-E8016",
        title: "Unsupported construct",
        rule: "ENGINEERING_LANGUAGE §7.A and §9",
        meaning: "The fragment has no rule for the construct: a `kind_products` row is missing for the operand kinds, an operand carries the `nominal` claim into a rule defined only for `exact`/`enclosure`, or a rule identifier is unknown. Nothing invents a bound.",
        next_action: "Declare the required `kind_products` row, supply `exact`/`enclosure` operands, or choose a rule the library supports.",
    },
    DiagnosticExplanation {
        code: "CORE-E8017",
        title: "Obligation unmet",
        rule: "ENGINEERING_LANGUAGE §6 and §8.2",
        meaning: "A generated obligation is open: nothing refuted it, but no rule or admissible premise discharged it — for example `independent(S)` with no attested premise. Open obligations block planning of dependents.",
        next_action: "Supply an attributed premise or evidence that discharges the obligation, or restructure so the obligation is not required.",
    },
    DiagnosticExplanation {
        code: "CORE-E8018",
        title: "Obligation refuted",
        rule: "ENGINEERING_LANGUAGE §6 and §7.E4",
        meaning: "A generated obligation is refuted by the record itself — for example `provenance_disjoint(S)` over inputs sharing a recorded source edge. Dependents cannot use the obligation's conclusion.",
        next_action: "Supply inputs with disjoint recorded provenance, or drop the method's disjointness admission policy if it is not required.",
    },
    DiagnosticExplanation {
        code: "CORE-E8019",
        title: "Precondition refuted",
        rule: "ENGINEERING_LANGUAGE §6",
        meaning: "A method `requires` clause provably fails on the bound entities — an operating domain outside material applicability, for example. The method does not apply; this is different from merely unmet.",
        next_action: "Bind entities satisfying the method's declared domain, or select a method whose applicability covers them.",
    },
    DiagnosticExplanation {
        code: "CORE-E8020",
        title: "Malformed import",
        rule: "ENGINEERING_LANGUAGE §9",
        meaning: "An `import` is missing a declared field — its value, its `assumptions` set, or a certificate's replayable payload. An absent `assumptions` field is not an empty set; the author has not stated the premise set at all.",
        next_action: "Declare the import's full residual assumption set explicitly — `assumptions: []` asserts unconditional.",
    },
    DiagnosticExplanation {
        code: "CORE-E8021",
        title: "Open hole",
        rule: "ENGINEERING_LANGUAGE §4 and §8.2",
        meaning: "An `input` was declared but never bound, or a `hole` term stays open. Holes are inspectable but not plannable; dependents are not evaluated on them.",
        next_action: "Bind the input, mark it `unavailable`, or author the term the hole stands for.",
    },
    DiagnosticExplanation {
        code: "CORE-E8022",
        title: "Ambiguous goal",
        rule: "ENGINEERING_LANGUAGE §4",
        meaning: "A `goal` admits more than one library method producing the required type. The candidates differ semantically — different inputs and assumptions — so the choice requires an authored `apply`.",
        next_action: "Replace the `goal` with an `apply` naming the intended method; the finding lists the candidates.",
    },
    DiagnosticExplanation {
        code: "CORE-E8023",
        title: "Lifecycle conflict",
        rule: "ENGINEERING_LANGUAGE §10.2",
        meaning: "Two supplied lifecycle entries give the same key different states. Iteration order never decides; the conflict is refused at bind.",
        next_action: "Supply a single lifecycle state per key; equal-state duplicates collapse harmlessly.",
    },
    DiagnosticExplanation {
        code: "CORE-E8024",
        title: "Missing argument",
        rule: "ENGINEERING_LANGUAGE §4",
        meaning: "An `apply` does not supply an argument for a slot the method signature declares.",
        next_action: "Bind every declared slot in `arguments`.",
    },
    DiagnosticExplanation {
        code: "CORE-E8025",
        title: "Undeclared projection",
        rule: "ENGINEERING_LANGUAGE §2.2 and §7",
        meaning: "A method output would carry a relation key its signature did not declare, or a signature drops a carried relation key without listing it in `projects`. A projection is only legal where the signature says so.",
        next_action: "Declare the relation on the signature's output, or list the dropped key in `projects`.",
    },
    DiagnosticExplanation {
        code: "CORE-E8026",
        title: "Lifecycle refused",
        rule: "ENGINEERING_LANGUAGE §10.2",
        meaning: "An applicable lifecycle entry — library scope or method scope — is `expired` or `withdrawn`. Refusals union: a method-level `active` never overrides a library-level refusal, and all refusal reasons are reported.",
        next_action: "Pin a current revision of the library or method, or update the lifecycle material.",
    },
    DiagnosticExplanation {
        code: "CORE-E8027",
        title: "Boundedness exceeded",
        rule: "ENGINEERING_LANGUAGE §11",
        meaning: "Elaboration or evaluation exceeded a declared profile bound — expression depth, term count, step count, witness-chain size, or operand-set size. The analysis stops with this finding rather than consuming unbounded resources.",
...
mod tests {
    ...
    fn every_code_the_compiler_source_emits_is_declared() {
        let source = [
            ...
            include_str!("language/analyze.rs"),
        ]
        .concat();
