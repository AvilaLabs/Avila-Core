# Business-model hypotheses

Nothing in this document is validated pricing or a revenue forecast. It defines
the economic design that Core must test without compromising the trust model.

## What the customer buys

The customer is not buying a favorable answer. It is buying a properly bounded,
policy-compliant resolution attempt whose result may be:

- `PASS`: the requirement was established inside the declared boundary;
- `FAIL`: the requirement was contradicted inside the declared boundary; or
- `INCONCLUSIVE`: the admitted methods and evidence could not decide it.

The same agreed commercial obligation is satisfied by any of those states when
the evidence contract and delivery policy were followed. `NOT_EVALUATED` is not
a billable completion state unless the contract explicitly defines a paid
screening phase.

## Revenue layers

### 1. Core Local — adoption layer

Candidate contents:

- local/headless contract runner;
- contract and capability SDK;
- schema validator;
- evidence viewer and independent verifier;
- a small set of public specimen and reference capabilities.

Likely price: free. The final license remains open. The purpose is bottom-up
adoption, provider development, transparent evaluation, and protection against
evidence lock-in.

### 2. Core Enterprise — near-term commercial layer

Candidate paid capabilities:

- private capability and contract registries;
- organization evidence policy;
- identity, roles, approvals, and countersignatures;
- campaign collaboration and controlled review queues;
- change impact and selective rerun management;
- deployment, retention, audit, signing, and security controls;
- support for on-premises and air-gapped environments; and
- validated release channels for approved capability packages.

Candidate commercial unit: annual organization subscription based on governed
teams or deployment scope, not solver jobs or core-hours. Enterprise pricing pays
for control, coordination, and support while allowing the runtime to minimize
compute.

### 3. Evidence contracts — outcome-aligned transaction layer

For defined contract templates, Core may charge a fixed or bounded fee for a
policy-compliant resolution. The fee must be quoted from an admissibility and
effort model before execution. It may include third-party capability, data,
review, and compute costs.

Pricing dimensions could include:

- contract class and decision criticality;
- number and qualification level of required capabilities;
- data preparation and review burden;
- uncertainty and coverage obligations;
- turnaround and resource reservation; and
- archival and countersignature requirements.

Price must not depend on whether the answer is favorable.

### 4. Core Network — provider market layer

Qualified capability providers may set license, per-contract, or service fees.
Core handles discovery, policy matching, routing, metering where necessary,
evidence receipts, settlement, and disputes. Avila earns a transparent network or
contract-orchestration fee.

Provider compensation may reward:

- admitted use of a versioned capability;
- completion of a defined review or service obligation;
- quality, availability, and reproducibility history; and
- maintained qualification evidence within a declared scope.

It must not reward `PASS`, excessive compute, hidden lock-in, or suppressing an
inconclusive result.

### 5. Attestation services — later, conditional layer

If evidence consumers recognize Core packages, independent professionals or
organizations could countersign that a named process and policy were followed.
Avila may provide the mechanism and marketplace, but should not claim that a
Core-generated package is a regulatory certification.

This layer must wait for legal analysis, recognized reviewers, and externally
validated demand.

## Why the economics may compound

Contract definition, method qualification, adapter development, review logic,
and evidence mappings are expensive the first time. If governed correctly, they
become reusable capital:

- templates reduce scoping and sales cost;
- capability competition reduces single-provider bottlenecks;
- cached evidence and dependency-aware invalidation reduce rerun cost;
- portable packages reduce review preparation;
- qualification evidence amortizes across eligible contracts; and
- network volume creates distribution for specialist providers.

Core can therefore become faster and cheaper without making its service less
rigorous. That is the central economic hypothesis.

## Early revenue sequence

1. **Paid design-partner pilot:** charge for resolving one tightly bounded
   contract with substantial hands-on support. Separate service hours from the
   future product price so consulting effort is visible.
2. **Repeatable contract template:** quote a fixed or bounded price for the same
   class of question at a second organization.
3. **Enterprise control plane:** charge for private governance once multiple
   people and campaigns need shared policy.
4. **External capability payment:** prove one independent professional or
   provider can earn revenue through Core.
5. **Network transaction:** route one contract between organizations without
   Avila manually performing the technical work.

Skipping directly to marketplace economics would hide whether the underlying
contract is valuable.

## Unit economics to measure

- customer time from question to accepted package;
- total cost per resolved contract;
- Avila labor hours per contract;
- provider, data, license, review, and compute costs;
- fraction of evidence reused after a change;
- reviewer time and number of clarification loops;
- percentage of contracts ending in each verdict state;
- repeat rate by contract template;
- number of substitutable providers per capability type; and
- gross margin excluding one-time capability development.

Core has not reached product economics if margin depends on hidden founder labor.

